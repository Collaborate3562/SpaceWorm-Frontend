...
function MarketPlacePage() {
  ...
  // const mainnetContract = "0x5dbe62d89c20fed696580a046b82520e9f7830ad"; //deployed in mainnet
  const mainnetContract = "0x0CBe23a89aC72A0cD612CcD499B8c5Fe54B2bd1D";
  ...
  const mint_NFT = async (values) => {
    ...
    if (values.length > 0) {
      ...
      if(allowance < 2*10e17) {
        setMMStatusInfo("You must approve BUSD.");
        setCurrentModal("loading-screen");
        setModalOpen(true);
        return;
      }
  ...
  const approveBUSDHandler = async () => {
    ...
    transactionParameters.data = await BUSDContract.methods.approve(mainnetContract, web3.utils.toHex(eggPrice*10e17)).encodeABI();
    ...
    return (
      <>
        ...
        <div className="absolute mintegg-cancel-button">
          ...
          <button className="button" onClick={approveBUSDHandler}>
            approve
          </button>
        </div>
      </>
